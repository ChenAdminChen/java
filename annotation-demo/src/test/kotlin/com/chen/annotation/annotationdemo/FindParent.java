...
//        map.forEach((key, it) -> {
//
//            it.setValue(find(map, it));
//        });
//
//        List<Parent> list = map.values().stream().collect(Collectors.toList());
//
//        map.forEach((key, it) -> {
...
//            }
//        });
        readJava.fixFindParent(new ArrayList<>(map.values()));

        long end = System.currentTimeMillis();

        System.out.printf("took %d ms, %d\n", end - start, readJava.counter);
